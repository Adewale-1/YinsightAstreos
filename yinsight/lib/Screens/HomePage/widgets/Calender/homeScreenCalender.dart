...
class _CalendarSectionState extends ConsumerState<CalendarSection> {
  ...
  // Utility function to check if two DateTime objects are on the same day
  ...
  // // Define a function to display the tasks for a specific hour
  Widget _addTaskDisplayOnCalendar(bool isHighlighted, DateTime timeForSlot, List<Task> tasksForThisHour) {
  return Container(
        ...
        ListTile(
          leading: Text(
            ...
            style: GoogleFonts.lexend(
              textStyle: const TextStyle(color: Colors.black, fontSize: 10.0, fontWeight: FontWeight.w600),
            ),
          ),
          title: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: tasksForThisHour.where((task) => task.assignedTime != null && isSameHour(task.assignedTime!, timeForSlot)).map((task) {
...
bool isSameHour(DateTime dt1, DateTime dt2) {
  ...
  Future<void> fetchTasksForSelectedDay(WidgetRef ref, DateTime date) async {
    ...
    final formattedDate = DateFormat('yyyy-MM-dd').format(date); // Ensure format is correct
    final uri = Uri.parse('${UserInformation.getRoute('getDateFilteredTasksInHomeCalendar')}?date=$formattedDate');
    ...
    if (response.statusCode == 200) {
      ...
      List<Task> tasks = (jsonResponse['tasks'] as List).map((task) => Task.fromJson(task)).toList();
  ...
  Widget build(BuildContext context) {
    ...
    final Map<DateTime, List<Task>> calendarTasks = ref.watch(calendarTasksProvider);
                ...
                children: [
                    ...
                    child: RichText(
                      text: TextSpan(
                        children: [
                          TextSpan(
                            text: "$formattedDate ",
                            style: const TextStyle(color: Colors.black, fontSize: 13.0, fontWeight: FontWeight.bold),
                          ),
                          TextSpan(
                            text: isToday ? 'Today' : '',
                            style: const TextStyle(color: Colors.blue, fontSize: 13.0, fontWeight: FontWeight.bold),
                          ),
                        ],
                      ),
                    ),
                  ...
                  IconButton(
                    ...
                    onPressed: isToday ? null : () {
                      DateTime newDate = selectedDate.subtract(const Duration(days: 1));
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (BuildContext context, int index) {
                  final hour = index;
                  final timeForSlot = DateTime(selectedDate.year, selectedDate.month, selectedDate.day, hour);
                  ...
                  return DragTarget<Task>(
                    ...
                    onAccept: (Task task) async {
                      ...
                      final updatedTask = task.copyWith(
                        assignedDate: DateTime(selectedDate.year, selectedDate.month, selectedDate.day),
                         assignedTime: timeForSlot,
                        
                      );

                      // Use updatedTask to generate JSON for sending to the backend
                      final taskJson = updatedTask.toJson(); // Use updatedTask here instead of task
                      ...
                      if (token == null) throw Exception('No token found');
                                ...
                                // Writing to a file
                      // final file = File('/Users/adewaleadenle/Software Development/GitHub Projects/Yinsight/yinsight/lib/Globals/myToken.txt');
                      ...
                      final response = await http.post(
                        Uri.parse(UserInformation.getRoute('addTaskToHomeCalendar')),
                        headers: {
                      ...
                      if (response.statusCode == 200) {
                       ...
                       ref.read(calendarTasksProvider.notifier).addTaskToCalendar(updatedTask, timeForSlot);
                    ...
                    builder: (BuildContext context, List<Task?> incoming, List rejected) {
                      ...
                      return _addTaskDisplayOnCalendar(isHighlighted, timeForSlot, tasksForThisHour);
                    },
                  );
                },
              ),
